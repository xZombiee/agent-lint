import { getAlternativeTools, getInstalledToolKeys, getToolDefinition } from "../utils/supportedTools.js";
...
export function toolMismatch(context) {
    ...
    for (const instructionFile of context.instructionFiles) {
        for (const mention of instructionFile.toolMentions) {
            ...
            const alternatives = getAlternativeTools(mention.tool).filter((tool) => installedTools.has(tool.key));
