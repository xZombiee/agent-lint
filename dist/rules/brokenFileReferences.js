...
function referenceExists(candidates, reference, context) {
    const collection = reference.target === "file"
        ? context.repoFiles
...
function hasEquivalentIgnoredArtifactPath(context) {
    return context.gitIgnoreRules.some((rule) => {
        ...
        const candidatePath = path.posix.join(rule.basePath, normalizedPattern.replace(/\/+$/u, ""));
        return !isTrackedPath(candidatePath, target, context.trackedPaths);
    });
}
function groupReferencesByLine(references) {
...
function buildInfoMessage(kind) {
    switch (kind) {
        case "example":
...
function buildInfoRepoFact(kind) {
    switch (kind) {
        case "example":
...
function buildInfoSuggestion(kind) {
    switch (kind) {
        case "example":
...
export function brokenFileReferences(context) {
    ...
    for (const instructionFile of context.instructionFiles) {
        ...
        for (const references of groups.values()) {
            ...
            for (const reference of references) {
                const candidates = resolveReferenceCandidates(instructionFile.path, reference);
                if (referenceExists(candidates, reference, context) ||
                    hasSafeIgnoredCandidate(candidates, reference, context)) {
                    continue;
                }
...
//# sourceMappingURL=brokenFileReferences.js.map