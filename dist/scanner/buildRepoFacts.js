...
function parseDeclaredPackageManager(value) {
...
function collectPackageManagers(repoFiles, packageJson) {
    const lockfiles = {
        npm: [],
        pnpm: [],
        yarn: [],
    ...
    const result = {
        lockfiles,
        workspaceFiles: repoFiles.filter((repoFile) => /(^|\/)(pnpm-workspace\.yaml|lerna\.json|rush\.json|turbo\.json|nx\.json)$/u.test(repoFile)),
    };
...
function collectTools(repoFiles, packageJson) {
    ...
    for (const tool of SUPPORTED_TOOLS) {
        ...
        const configFiles = repoFiles.filter((repoFile) => TOOL_CONFIG_PATTERNS[tool.key].some((pattern) => pattern.test(repoFile)));
...
async function readOptionalText(projectRoot, repoPath) {
...
function addRuntimeFact(runtimes, runtime, source, version) {
...
async function collectRuntimes(projectRoot, repoFiles, packageJson) {
    const runtimes = {};
    ...
    if (repoFiles.includes(".tool-versions")) {
        ...
        for (const line of content?.split(/\r?\n/u) ?? []) {
            ...
            addRuntimeFact(runtimes, tool.startsWith("node") ? "node" : tool, ".tool-versions", version);
        }
    }
    if (repoFiles.includes("mise.toml")) {
        ...
        for (const line of content?.split(/\r?\n/u) ?? []) {
            ...
            if (match?.groups?.tool && match.groups.version) {
                addRuntimeFact(runtimes, match.groups.tool, "mise.toml", match.groups.version);
            }
        }
    }
    return runtimes;
}
function collectCiProviders(repoFiles) {
...
async function collectCiFacts(projectRoot, repoFiles) {
    const githubWorkflowFiles = repoFiles.filter((repoFile) => /^\.github\/workflows\/[^/]+\.ya?ml$/u.test(repoFile));
    const githubWorkflowNames = new Set();
    const githubJobIds = new Set();
...
export async function buildRepoFacts(projectRoot, repoFiles, packageJson) {
    return {
...
//# sourceMappingURL=buildRepoFacts.js.map