...
export function classifyReferenceContext(
  line: string,
  section?: string,
): ReferenceKind {
  const context = normalizeContextValue(
    section ? `${section}\n${line}` : line,
  );

  if (
    /\b(git-ignored|ignored|temporary|artifacts?|reports?|generated artifacts?|scratch|cache)\b/u.test(
      context,
    ) ||
  ...
  if (
    /\b(such as|for example|e\.g\.|for instance|or another|one of)\b/u.test(context)
  ) {
    return "example";
  }
...
export function detectPathTargetKind(rawPath: string): PathTargetKind {
  if (rawPath.endsWith("/")) {
    return "dir";
  }
